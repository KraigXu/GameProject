...
namespace Verse
{
	...
	{
		// Token: 0x06000590 RID: 1424 RVA: 0x0001B87D File Offset: 0x00019A7D
		public static IEnumerable<StatDrawEntry> SpecialDisplayStats(HediffStage stage, Hediff instance)
		{
			...
			{
				yield return new StatDrawEntry(StatCategoryDefOf.Basics, "BleedingRate".Translate(), instance.BleedRate.ToStringPercent() + "/" + "LetterDay".Translate(), "Stat_Hediff_BleedingRate_Desc".Translate(), 4040, null, null, false);
			}
			float num = 0f;
			if (instance != null)
			{
			...
			if (stage != null)
			{
				...
				if (stage.makeImmuneTo != null)
				{
					...
					select im.label).ToCommaList(false).CapitalizeFirst(), "Stat_Hediff_PreventsInfection_Desc".Translate(), 4050, null, null, false);
				}
