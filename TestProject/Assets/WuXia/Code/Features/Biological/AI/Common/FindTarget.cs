...
namespace GameSystem.AI
{
    ...
    public static class FindTarget
    {
        ...
        static public Transform FindEnemy(out RaycastHit hit, Vector3 source,Vector3 direction, GameObject player, Quaternion angle, float radius, Color debugColor)
        {
        ...
        static public bool HitEnemy(RaycastHit hit, GameObject player)
        {
            if (hit.collider.CompareTag("Enemy") && hit.collider.gameObject != player)
            {
                return true;
            }
            return false;
