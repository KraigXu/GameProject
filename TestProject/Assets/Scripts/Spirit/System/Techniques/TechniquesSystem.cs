...
namespace GameSystem
{
    ...
    public class TechniquesSystem : ComponentSystem
    {
        ...
        public static void SpawnTechnique(Entity target, int id,EntityManager manager)
        {

            manager.AddComponent(target, typeof(TechniquesProperty));
            manager.SetComponentData(target, new TechniquesProperty
            {
                IncreaseRate = 3,
                LowerRate = 1
            });
            ...
            for (int i = 0; i < articleDatas.Count; i++)
            {
                ...
                Entity entity = manager.CreateEntity(GameSceneInit.TechniquesArchetype);

                manager.SetComponentData(entity, new Techniques
                {
                    BiologicalTarget = target,
                    Level = 3,
