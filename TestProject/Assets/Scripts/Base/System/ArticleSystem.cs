...
namespace GameSystem
{
    public class ArticleSystem : ComponentSystem
    {
        ...
        public static void SettingArticleFeature(Entity targetEntity, int dataid,EntityManager manager)
        {

            manager.AddComponentData(targetEntity, new Knapsack
            {
                UpperLimit = 1000000,
                KnapscakCode = 111,
                CurUpper = 10,
            ...
            {
                var articleData = articleDatas[i];
                Entity entity = manager.CreateEntity(_articleArchetype);

                List<KeyValuePair<ENUM_ITEM_ATTRIBUTE, string>> valuePairs = JsonConvert.DeserializeObject<List<KeyValuePair<ENUM_ITEM_ATTRIBUTE, string>>>(articleData.Value);

                manager.SetComponentData(entity, new ArticleItem
                {
                    BiologicalEntity = targetEntity,
                    GuiId = articleData.Id,
                    Count = articleData.Count,
                ...
                switch ((ENUM_ITEM_CLASS)articleData.Type)
                {
                    case ENUM_ITEM_CLASS.ITEM_CLASS_WEAPON6:
                        {
                            AttackProperty attackProperty = JsonConvert.DeserializeObject<AttackProperty>(articleData.Text);
                            manager.SetComponentData(entity, attackProperty);
                        }
                        break;
                    case ENUM_ITEM_CLASS.ITEM_CLASS_WEAPON7:
                        {
