...
public static class QuestUtility
{
	...
	{
		if (!Find.QuestManager.IsReservedByAnyQuest(pawn) && (QuestGen.quest == null || !QuestGen.quest.QuestReserves(pawn)))
		{
			return QuestGen.WasGeneratedForQuestBeingGenerated(pawn);
		}
		return true;
	}
	...
	public static IEnumerable<QuestPart_WorkDisabled> GetWorkDisabledQuestPart(Pawn p)
	{
		...
		for (int j = 0; j < quests.Count; j++)
		{
			...
			for (int i = 0; i < partList.Count; i++)
			{
				...
				if ((questPart_WorkDisabled = (partList[i] as QuestPart_WorkDisabled)) != null && questPart_WorkDisabled.pawns.Contains(p))
				{
