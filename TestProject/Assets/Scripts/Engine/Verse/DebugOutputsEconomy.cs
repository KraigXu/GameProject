...
public static class DebugOutputsEconomy
{
	...
	public static void Drugs()
	{
		...
		HediffDef Addiction(ThingDef d)
		{
			if (!Addictive(d))
			{
				return null;
		...
		bool IsDrug(ThingDef d)
		{
			return d.HasComp(typeof(CompDrug));
		}
		float LargeOverdoseChance(ThingDef d)
		{
			if (IsDrug(d))
			{
		...
		float MinToleranceToAddict(ThingDef d)
		{
			if (IsDrug(d))
			{
		...
		float NewAddictionChance(ThingDef d)
		{
			if (IsDrug(d))
			{
		...
		float NewAddictionSeverity(ThingDef d)
		{
			if (IsDrug(d))
			{
		...
		float OldAddictionSeverityOffset(ThingDef d)
		{
			if (IsDrug(d))
			{
		...
		FloatRange OverdoseSeverity(ThingDef d)
		{
			if (IsDrug(d))
			{
