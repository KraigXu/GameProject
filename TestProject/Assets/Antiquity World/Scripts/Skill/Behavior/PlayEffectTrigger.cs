    ...
    public class PlayEffectTrigger : SkillBehavior
    {
        ...
        public override bool Execute(ISkillTrigger instance, float curTime, SkillInstance controller)
        {
            ...
            if (curTime >= m_StartTime && m_IsExected == false)
            {
                ...
                if (EffectType == 0)
                {
                    WXPoolManager.Pools[Define.PoolName].Spawn(skillData.Prefab,controller.transform.position,controller.transform.rotation);
                }
                else if(EffectType==1)
                {
                    WXPoolManager.Pools[Define.PoolName].Spawn(skillData.Prefab, controller.TargetPos);
                }
                m_IsExected = true;
                return true;
            }
