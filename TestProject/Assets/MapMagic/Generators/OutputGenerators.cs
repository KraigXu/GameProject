	...
	public class TexturesOutput : OutputGenerator
	{
		...
		public class Layer
		{
			...
			public void OnGUI (Layout layout, bool selected, int num) 
			{
				...
				#else
				layout.Label("Unity 2018.3 Terrain Layer");
				#endif
...
//					if (layer.prefab == null) continue;
    ...
    [System.Serializable]
    [GeneratorMenu(menu = "Output", name = "Citys", disengageable = true)]
    public class CityOutput : OutputGenerator
    {
        ...
        public static void Process(CoordRect rect, Chunk.Results results, GeneratorsAsset gens, Chunk.Size terrainSize, Func<float, bool> stop = null)
        {
            ...
            foreach (CityOutput gen in gens.GeneratorsOfType<CityOutput>(onlyEnabled: true, checkBiomes: true))
            {
                Matrix biomeMask = null;
                if (gen.biome != null)
                {
                ...
                allGensMasks.Add(new TupleSet<CityOutput, Matrix>(gen, biomeMask));
            }
            int allGensMasksCount = allGensMasks.Count;
            //Debug.Log(allGensMasksCount);
            ...
            for (int g = 0; g < allGensMasksCount; g++)
            {
                CityOutput gen = allGensMasks[g].item1;

                //iterating in layers
                for (int b = 0; b < gen.baseLayers.Length; b++)
                {
                    if (stop != null && stop(0)) return; //checking stop before reading output
                    CityOutput.Layer layer = gen.baseLayers[b];
                    if ((object)layer.prefab == null) continue;
            ...
            //queue apply
            if (stop != null && stop(0)) return;
            results.apply.CheckAdd(typeof(CityOutput), transitions, replace: true);
        }
        ...
        public static IEnumerator Apply(CoordRect rect, Terrain terrain, object dataBox, Func<float, bool> stop = null)
        {
            ...
            //clear all of non-included pools from rect
            MapMagic.instance.livingAreaPool.ClearAllRectBut(terrainRect, transitions);

            //remove empty pools
            MapMagic.instance.livingAreaPool.RemoveEmptyPools();
        ...
        public override void OnGUI(GeneratorsAsset gens)
        {
            if (MapMagic.instance != null)
            {
                layout.Toggle(ref MapMagic.instance.livingAreaPool.regardPrefabRotation, "Regard Prefab Rotation");
                layout.Toggle(ref MapMagic.instance.livingAreaPool.regardPrefabScale, "Regard Prefab Scale");
                layout.Toggle(ref MapMagic.instance.livingAreaPool.instantiateClones, "Instantiate Clones");
                layout.Field(ref biomeBlendType, "Biome Blend", fieldSize: 0.47f);
